    ...
    orders.map(async (order) => {
      return prisma.payment.create({
        data: {
          ...
          status: faker.helpers.arrayElement([
            'PENDING', 'COMPLETED', 'REFUND', 'FAILED', 'PROCCESSING'
          ]),
          paymentMethod: faker.helpers.arrayElement([
            'CREDITCARD', 'STRIPE', 'PAYPAL', 'BANKTRANSFER'
          ]),
      ...
      return prisma.pickup.create({
        data: {
          ...
          pickupType: faker.helpers.arrayElement([
            'AIRPORT', 'IN_PERSON', 'PICKUPPOINT', 'DELIVERY'
          ]),
          ...
          status: faker.helpers.arrayElement([
            'SCHEDULED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED', 'DELAYED', 'DELIVERED'
          ]),
    ...
    Array.from({ length: 20 }).map(async () => {
      return prisma.notification.create({
        data: {
          userId: faker.helpers.arrayElement(users).id,
          type: faker.helpers.arrayElement([
            'REQUEST', 'ACCEPTED', 'REJECTED', 'ORDER_CREATED',
            'PAYMENT_SUCCESS', 'PAYMENT_FAILED', 'PICKUP_SCHEDULE',
          ...
          requestId: faker.helpers.maybe(() =>
            faker.helpers.arrayElement(requests).id, { probability: 0.5 }),
          orderId: faker.helpers.maybe(() =>
            faker.helpers.arrayElement(orders).id, { probability: 0.5 }),
          pickupId: faker.helpers.maybe(() =>
            faker.helpers.arrayElement(pickups).id, { probability: 0.5 }),
        },
      });
    })
  );
    ...
    Array.from({ length: 5 }).map(async () => {
      return prisma.subscription.create({
        data: {
          ...
          type: faker.helpers.arrayElement([
            'STREAMING', 'SOFTWARE', 'GAMING', 'EDUCATION', 'OTHER'
          ]),
          categoryId: faker.helpers.arrayElement(categories).id,
      ...
      return prisma.message.create({
        data: {
          ...
          mediaId: faker.helpers.maybe(() =>
            faker.helpers.arrayElement(media).id, { probability: 0.3 }),
    ...
    orders.map(async (order) => {
      return prisma.goodsProcess.create({
        data: {
          orderId: order.id,
          status: faker.helpers.arrayElement([
            'INITIALIZED', 'CONFIRMED', 'PAID',
            'IN_TRANSIT', 'PICKUP_MEET', 'FINALIZED'
          ]),
        },
      });
    })
