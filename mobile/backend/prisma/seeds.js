...
if (!fs.existsSync(envPath)) {
  ...
  console.log(
    ".env file created. Please edit it with your actual database credentials."
  );
  ...
  try {
    ...
    const sponsorships = await Promise.all(
      Array.from({ length: 5 }).map(() => {
        ...
        const recipients = faker.helpers.arrayElements(users, {
          min: 1,
          max: 3,
        });
    ...
    const notifications = await Promise.all(
      Array.from({ length: 20 }).map(() => {
        ...
        const sender = faker.helpers.maybe(
          () =>
        ...
        return prisma.notification.create({
          data: {
            ...
            request: faker.datatype.boolean()
              ? { connect: { id: faker.helpers.arrayElement(requests).id } }
              : undefined,
            order: faker.datatype.boolean()
              ? { connect: { id: faker.helpers.arrayElement(orders).id } }
              : undefined,
            pickup: faker.datatype.boolean()
              ? { connect: { id: faker.helpers.arrayElement(pickups).id } }
              : undefined,
          },
        });
      })
    ...
    const messages = await Promise.all(
      Array.from({ length: 20 }).map(() => {
        ...
        const ticket = faker.datatype.boolean()
