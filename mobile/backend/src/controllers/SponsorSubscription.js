...
const sponsor = {
  createSponsorShip: async (req, res) => {
    ...
    try {
      const { description, price, duration, platform, categoryId, amount } =
        req.body;
  ...
  createSubscription: async (req, res) => {
    try {
      const { name, description, price, duration, type, categoryId, isActive } =
        req.body;
  ...
  allSub: async (req, res) => {
    try {
      const subscriptions = await prisma.sponsorship.findMany({
