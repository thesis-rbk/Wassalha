...
const notificationHandlers = (socket) => {
  ...
  socket.on("offer_cancelled", async ({ requesterId, offerDetails }) => {
    ...
    if (parseInt(offerDetails.travelerId) !== socket.user.id) {
      console.log(
        `⚠️ Security warning: User ${socket.user.id} tried to cancel offer as traveler ${offerDetails.travelerId}`
      );
      return;
    }
    ...
    const roomName = `user_${requesterId}`;
    console.log(
      `🚫 Sending offer cancellation notification to room: ${roomName}`,
      {
        requesterId,
        offerDetails,
      }
    ...
    try {
      const notification = await prisma.notification.create({
        data: {
          ...
          status: "UNREAD",
          requestId: offerDetails.requestId
            ? parseInt(offerDetails.requestId)
            : null,
