  ...
  const fetchRequests = async () => {
    try {
      ...
      const filteredRequests = allRequests.filter((request: Request) => {
        ...
        console.log(
          `Request ${request.id} status: ${request.status}, isPending: ${isPending}`
        );
        ...
        console.log(
          `Request ${request.id} hasOrder: ${!!request.order}, orderStatus: ${
            request.order?.orderStatus
          }, hasNoActiveOrder: ${hasNoActiveOrder}`
        );
      ...
      console.log(
        `Filtered from ${allRequests.length} to ${filteredRequests.length} requests`
      );
